...
def add_historical_delay_features(df: DataFrame) -> DataFrame:
    train_df = df.filter(F.col("split") == "train")
    if train_df.limit(1).count() == 0:
        raise ValueError("Training split is empty. Historical delay features require at least some train data.")

    global_delay_rate_row = train_df.agg(
        F.avg(F.col("label_delay_15").cast("double")).alias("global_delay_rate")
    ).collect()[0]
    global_delay_rate = float(global_delay_rate_row["global_delay_rate"])

    origin_hist = (
        train_df.groupBy("Origin")
        .agg(F.avg(F.col("label_delay_15").cast("double")).alias("origin_hist_delay_rate"))
    )

    route_hist = (
        train_df.groupBy("route")
        .agg(F.avg(F.col("label_delay_15").cast("double")).alias("route_hist_delay_rate"))
    )

    enriched = (
        df.join(origin_hist, on="Origin", how="left")
          .join(route_hist, on="route", how="left")
          .fillna(
              {
                  "origin_hist_delay_rate": global_delay_rate,
                  "route_hist_delay_rate": global_delay_rate,
              }
          )
    )
...
def select_feature_columns(df: DataFrame) -> DataFrame:
    return df.select(
        ...
        F.col("route"),
        F.col("carrier_route"),
        F.col("origin_hist_delay_rate").cast("double"),
        F.col("route_hist_delay_rate").cast("double"),
...
def build_features(df: DataFrame) -> DataFrame:
    ...
    filtered = filtered.filter(F.col("Year").isin(2022, 2023, 2024)).sample(False, 0.02, seed=42)  # dev-only subset for local feature engineering experiment
    ...
    transformed = add_historical_delay_features(transformed)
